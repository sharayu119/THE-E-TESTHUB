...
def search(request,pageno):
   ...
   # subject=request.GET["subject"]
    ...
    return render(request,'examapp/resultanalysis.html',{'results':queryset,'listofint':list})
...
def nextQuestion(request):
    ...
    current_index = request.session["qindex"]
    is_last_question = current_index >= len(questionlist) - 1
    
    if not is_last_question:
        request.session["qindex"] = current_index + 1
        ...
        return render(request, 'examapp/question.html', {
            ...
            'message': "Please check all answers and click endexam",
...
def endexam(request):
    ...
    dictionary = request.session["answers"]
    listoflist = dictionary.values()
    finalscore = 0  # Initialize score to 0
    
    # Calculate score for all answers
    for answer in listoflist:
        if answer[2] == answer[3]:  # if correct_answer == user_answer
            finalscore += 1
    ...
    return render(request, 'examapp/score.html', {
        'username': username,
        'score': finalscore,
